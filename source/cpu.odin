    ...
    switch size {
        case 0:
            if mode == 0 {
                ...
                ea_data := cpu_get_ea_data8(mode, reg, addr)
                cpu_prefetch()
                bus_write8(addr, ea_data - u8(data))
                cycles += 8
            }
        case 1:
            if mode == 0 {
                data := u32(u16(i16(D[reg]) - i16(data)))
                D[reg] &= 0xFFFF0000
                D[reg] |= data
                cpu_prefetch()
                cycles += 4
                ...
                ea_data := cpu_get_ea_data16(mode, reg, addr)
                cpu_prefetch()
                bus_write16(addr, ea_data - data)
                cycles += 8
            }
        case 2:
            if mode == 0 {
                data := u32(i32(D[reg]) - i32(data))
                D[reg] = data
                cpu_prefetch()
                cycles += 8
            } else if mode == 1 {
                ...
                ea_data := cpu_get_ea_data32(mode, reg, addr)
                cpu_prefetch()
                bus_write32(addr, ea_data - u32(data))
                cycles += 12
            }
    }
}
    ...
    switch size {
        case 0:
            ...
            if dir == 1 {
                ...
                data := u32(u8(i8(D[reg]) - i8(ea_data)))
                D[reg] &= 0xFFFFFF00
                D[reg] |= data
                cycles += 4
            }
        case 1:
            ...
            if dir == 1 {
                ...
                data := u32(u16(i16(D[reg]) - i16(ea_data)))
                D[reg] &= 0xFFFF0000
                D[reg] |= data
                cycles += 4
            }
        case 2:
            ...
            if dir == 1 {
                ...
                data := u32(i32(D[reg]) - i32(ea_data))
                D[reg] = data
                cycles += 6
                if mode <= 1 {
                    cycles += 2
...
cpu_eor :: proc(opcode: u16)
{
    ...
    switch size {
        ...
        case 1:
            ...
            if mode == 0 {
                data := u32(u16(D[reg]) ~ u16(ea_data))
                D[reg2] &= 0xFFFF0000
                D[reg2] |= data
                cycles += 4
            } else {
